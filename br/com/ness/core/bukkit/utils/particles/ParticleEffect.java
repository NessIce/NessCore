package br.com.ness.core.bukkit.utils.particles;
...
public enum ParticleEffect {
    ...
    private static boolean isLongDistance(final Location location, final List<Player> players) {
        for (final Player player : players) {
            if (player.getLocation().getWorld().equals(location.getWorld()) && player.getLocation().distanceSquared(location) < 65536.0) {
                continue;
            }
            return true;
        }
